...
CREATE OR REPLACE TRIGGER jg_cezb_observe
    BEFORE INSERT OR DELETE OR UPDATE OF konr_id, rcez_id, cena, jdmr_nazwa, grod_id, gras_id, typ, inma_id, cena_brutto
    ON ap_ceny_zbytu
    REFERENCING NEW AS NEW OLD AS OLD
    FOR EACH ROW
BEGIN
...
CREATE OR REPLACE TRIGGER jg_konr_observe
    BEFORE INSERT OR DELETE OR UPDATE OF id, aktualny, symbol, skrot, odbiorca, mail, nazwa, nr_umowy_ind, platnik, platnik_id, nr_tel, nip, nr_faksu, blokada_sprz, dni_do_zaplaty
    ON ap_kontrahenci
    REFERENCING NEW AS NEW OLD AS OLD
    FOR EACH ROW
BEGIN

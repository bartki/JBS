  ...
  VALUES
    (jg_sqre_seq.NEXTVAL,
     ...
     'SELECT rndo.symbol_dokumentu invoice_number,
               ...
               FROM rk_rozr_nal_dokumenty_vw rndo,
                    ap_kontrahenci konr
  ...
  VALUES
    (jg_sqre_seq.NEXTVAL,
     ...
     'SELECT konr.symbol customer_number,
       ...
       jg_output_sync.format_number(lg_knr_likr_sql.aktualny_limit_konr_kwota(konr.id,
                                                                              pa_sesja.dzisiaj),
                                    2) credit_limit,
       (SELECT grupa
  ...
  VALUES
    (jg_sqre_seq.NEXTVAL,
     ...
     'SELECT header.symbol invoice_symbol,
       ...
       CASE
         WHEN header.gross_value <= lg_dosp_sql.kwota_zaplat_na_dok(id) THEN
          ''T''
         ELSE
          ''N''
       END is_paid,
  ...
  VALUES
    (jg_sqre_seq.NEXTVAL,
     ...
     'SELECT inma_kpl.indeks set_id,
       ...
       jg_output_sync.format_number(lg_stm_sgpu_sql.stan_goracy(inma_kpl.id,
                                                                inma_kpl.jdmr_nazwa,
                                                                NULL),
                                    100) available_stock,
